...
def grid_hash(col, row):
    return ((row * 31) ^ col) & (GRID_BUCKETS - 1)
...
def main():
    ...
    areas = sorted(
        p for p in MAPS_DIR.iterdir() if p.is_dir() and not p.name.startswith(".")
    )
    if not areas:
    ...
    for area in areas:
        ...
        for col, row in all_cells:
            ...
            entry = f"{name}_GENTRY_{col}_{row}"
            lines += [
                f"    {entry}.next = GRID_{name}.cells[{bucket}];",
